    ...
    animate: function(a, b, c, d, e) {
      return this.each(function() {
        function o() {
          (f.called = !0),
            (f.CSSAIsRunning = !1),
            g.off(jQuery.CSS.transitionEnd + "." + f.id),
            clearTimeout(f.timeout),
            g.css(jQuery.CSS.sfx + "transition", ""),
            "function" == typeof e && e.apply(f),
            "function" == typeof f.CSSqueue &&
              (f.CSSqueue(), (f.CSSqueue = null));
        }
        var f = this,
          g = jQuery(this);
        ...
        if (
          ((f.CSSqueue = null), (f.eventType = h.type), 0 !== g.length && a)
        ) {
          if (
          ...
          if (
            ...
            for (var j in a) {
              if (
                ("transform" === j && delete a[j],
                "filter" === j && delete a[j],
                "transform-origin" === j && delete a[j],
                "auto" === a[j] && delete a[j],
                "x" === j)
              ) {
                var k = a[j],
  ...
  (a.simpleSlider = {
    ...
    init: function(b) {
      return this.each(function() {
        ...
        e.addClass("simpleSlider"),
          (d.opt = {}),
          a.extend(d.opt, a.simpleSlider.defaults, b),
    ...
    updateSliderVal: function(b) {
      ...
      var c = this,
        d = c.get(0);
      d.opt.initialval =
        "number" == typeof d.opt.initialval
          ? d.opt.initialval
          : d.opt.initialval(d);
      ...
      (d.x =
        "object" == typeof b
          ...
          : "number" == typeof b
          ? (b * e) / d.opt.scale
          : (d.opt.initialval * e) / d.opt.scale),
        (d.y =
          "object" == typeof b
            ? b.clientY + document.body.scrollTop - c.offset().top
            : "number" == typeof b
            ? ((d.opt.scale - d.opt.initialval - b) * f) / d.opt.scale
            : (d.opt.initialval * f) / d.opt.scale),
        (d.y = c.outerHeight() - d.y),
        (d.scaleX = (d.x * d.opt.scale) / e),
        (d.scaleY = (d.y * d.opt.scale) / f),
        (d.outOfRangeX =
        ...
        (d.outOfRangeY =
          d.scaleY > d.opt.scale
            ...
            : 0),
        (d.outOfRange =
          "h" == d.opt.orientation ? d.outOfRangeX : d.outOfRangeY),
        (d.value =
          "undefined" != typeof b
            ? "h" == d.opt.orientation
              ? d.x >= c.outerWidth()
        ...
        "h" == d.opt.orientation
          ...
          : d.level.height(g(d.y, f)),
        "function" == typeof d.opt.callback && d.opt.callback(d);
    }
  }),
